				...
				if (req.body.Size)
				{
					...
					con.query(query_stmt, function (err, initialresult, initialfields) 
					{
						...
						if (initialresult.length == 1)
						{
							...
							var query_stmt = "select * from X_birds where ScientificName IN ("+sciNames+");";
									...
									for (var i = 0; i < result.length ; i++ )
									{
										if (result[i].Size > lower)
										{
											if (result[i].Size < upper)
											{
												...
												{
													console.log ("Result set based on Plumage: "+result[i].CommonName+" Plumage: "+result[i].Plumage);
